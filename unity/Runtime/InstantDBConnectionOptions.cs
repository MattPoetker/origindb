...
namespace InstantDB.Unity
{
    ...
    public class InstantDBConnectionOptions
    {
        ...
        /// </summary>
        public InstantDBConnectionOptions WithDefaults()
        {
        ...
        /// </summary>
        public static InstantDBConnectionOptions FromEnvironment()
        {
            return new InstantDBConnectionOptions
            {
                ServerUrl = Environment.GetEnvironmentVariable("INSTANTDB_SERVER_URL") ?? "http://localhost:8080",
                ModuleName = Environment.GetEnvironmentVariable("INSTANTDB_MODULE_NAME") ?? "default",
                AuthToken = Environment.GetEnvironmentVariable("INSTANTDB_AUTH_TOKEN"),
                EnableLogging = bool.Parse(Environment.GetEnvironmentVariable("INSTANTDB_ENABLE_LOGGING") ?? "true")
            };
        }

        public override string ToString()
        {
            return $"InstantDBConnectionOptions(ServerUrl='{ServerUrl}', ModuleName='{ModuleName}', AutoReconnect={AutoReconnect})";
        }
    }
}
#endif