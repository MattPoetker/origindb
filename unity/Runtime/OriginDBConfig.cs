...
{
    /// <summary>
    /// ScriptableObject configuration for OriginDB connections.
    /// Create instances of this asset to store connection settings.
    /// </summary>
    [CreateAssetMenu(fileName = "OriginDBConfig", menuName = "OriginDB/Configuration", order = 1)]
    public class OriginDBConfig : ScriptableObject
    {
        [Header("Server Settings")]
        [Tooltip("OriginDB server URL (supports HTTP/HTTPS, automatically converted to WebSocket)")]
        ...
        public OriginDBConnectionOptions CreateConnectionOptions()
        {
    ...
    /// </summary>
    public static class OriginDBConfigUtility
    {
        /// <summary>
        /// Creates a local development configuration.
        /// </summary>
        public static OriginDBConfig CreateLocalConfig()
        {
        ...
        /// </summary>
        public static OriginDBConfig CreateProductionConfig()
        {
            var config = ScriptableObject.CreateInstance<OriginDBConfig>();
        ...
        /// </summary>
        public static OriginDBConfig CreateCloudBuildConfig()
        {
            var config = ScriptableObject.CreateInstance<OriginDBConfig>();

            // Use environment variables if available
            config.serverUrl = Environment.GetEnvironmentVariable("ORIGINDB_SERVER_URL") ?? "http://localhost:8080";
            config.moduleName = Environment.GetEnvironmentVariable("ORIGINDB_MODULE_NAME") ?? "cloudbuild";
            config.authToken = Environment.GetEnvironmentVariable("ORIGINDB_AUTH_TOKEN") ?? "";
