...
namespace InstantDB.Unity.Editor
{
    /// <summary>
    /// Custom property drawer for InstantDBConfig to provide better Inspector experience.
    /// </summary>
    [CustomEditor(typeof(InstantDBConfig))]
    public class InstantDBConfigEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var config = target as InstantDBConfig;

            EditorGUILayout.Space();
            EditorGUILayout.LabelField("InstantDB Configuration", EditorStyles.boldLabel);
            EditorGUILayout.Space();
        ...
        private void TestConnection(InstantDBConfig config)
        {
            try
            {
                if (InstantDBNetworkManager.Instance != null)
                {
                    ...
                    Debug.Log($"[InstantDB Editor] Test connection created: {connection}");
                    EditorUtility.DisplayDialog("Test Connection",
                ...
                {
                    EditorUtility.DisplayDialog("Test Connection",
                        "No InstantDBNetworkManager found in the scene.", "OK");
                }
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"[InstantDB Editor] Test connection failed: {ex.Message}");
    ...
    /// <summary>
    /// Menu items for creating InstantDB assets and objects.
    /// </summary>
    public static class InstantDBMenuItems
    {
        [MenuItem("GameObject/InstantDB/Network Manager", false, 10)]
        public static void CreateNetworkManager(MenuCommand menuCommand)
        {
            var go = new GameObject("InstantDB Network Manager");
            go.AddComponent<InstantDBNetworkManager>();

            GameObjectUtility.SetParentAndAlign(go, menuCommand.context as GameObject);
            Undo.RegisterCreatedObjectUndo(go, "Create InstantDB Network Manager");
            Selection.activeObject = go;
        }

        [MenuItem("Assets/Create/InstantDB/Configuration")]
        public static void CreateConfiguration()
        {
            ...
            string assetPathAndName = AssetDatabase.GenerateUniqueAssetPath(path + "/InstantDBConfig.asset");
        ...
        [MenuItem("Assets/Create/InstantDB/Local Development Config")]
        public static void CreateLocalConfig()
        {
            var config = InstantDBConfigUtility.CreateLocalConfig();
            ...
            string assetPathAndName = AssetDatabase.GenerateUniqueAssetPath(path + "/InstantDBConfig_Local.asset");
        ...
        [MenuItem("Assets/Create/InstantDB/Production Config")]
        public static void CreateProductionConfig()
        {
            var config = InstantDBConfigUtility.CreateProductionConfig();
            ...
            string assetPathAndName = AssetDatabase.GenerateUniqueAssetPath(path + "/InstantDBConfig_Production.asset");
        ...
        [MenuItem("InstantDB/Documentation")]
        public static void OpenDocumentation()
        {
            Application.OpenURL("https://docs.instantdb.com/unity");
        }

        [MenuItem("InstantDB/Support")]
        public static void OpenSupport()
        {
            Application.OpenURL("https://github.com/your-org/instantdb/issues");
        }

        [MenuItem("InstantDB/About")]
        public static void ShowAbout()
        {
            EditorUtility.DisplayDialog("InstantDB for Unity",
                "Real-time database integration for Unity\n\nVersion: 1.0.0\nWebsite: https://instantdb.com", "OK");
        }
    }
}
#endif