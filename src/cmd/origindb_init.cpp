...
public static class Program
{
  ...
  <PropertyGroup>
    ...
    <!-- OriginDB runs core WASI p1 modules: net8.0 + wasi-wasm + the
  ...
  <!-- The OriginDB C# SDK is a single source file (no NuGet package yet).
       Either copy sdk/csharp/OriginDB.cs from the OriginDB repository into
       this directory (it is compiled automatically), or point at your
       checkout: dotnet publish -c Release -p:OriginDBSdkDir=/path/to/instant_db/sdk/csharp -->
  <ItemGroup Condition="'$(OriginDBSdkDir)' != ''">
    <Compile Include="$(OriginDBSdkDir)/OriginDB.cs" />
  </ItemGroup>
...
An OriginDB WASM module written in C#. Reducers run inside the OriginDB
server with transactional table access (WASM ABI v1 — see docs/WASM_ABI.md
in the OriginDB repository).
...
```bash
cp /path/to/instant_db/sdk/csharp/OriginDB.cs .
```

(or build with `-p:OriginDBSdkDir=/path/to/instant_db/sdk/csharp`).
...
```bash
origindb client call {{PROJECT_NAME_LOWER}} CreateUser '["Alice", "alice@example.com"]'
origindb client call {{PROJECT_NAME_LOWER}} GetUsers '[]'
```
...
See `sdk/csharp/README.md` in the OriginDB repository for the full API.
)CSREADME"},
...
if [ ! -f "OriginDB.cs" ] && [ -z "$ORIGINDB_SDK_DIR" ]; then
    echo "warning: OriginDB.cs not found in this directory." >&2
    echo "  Copy it from the OriginDB repo:  cp /path/to/instant_db/sdk/csharp/OriginDB.cs ." >&2
    echo "  Or set ORIGINDB_SDK_DIR=/path/to/instant_db/sdk/csharp" >&2
fi
...
EXTRA_ARGS=""
if [ -n "$ORIGINDB_SDK_DIR" ]; then
    EXTRA_ARGS="-p:OriginDBSdkDir=$ORIGINDB_SDK_DIR"
fi
...
if [ -f "$WASM_FILE" ]; then
    ...
    echo "Deploy with: origindb publish"
else
  ...
  "version": "1.0.0",
  "private": true,
  "description": "An OriginDB WASM module written in AssemblyScript",
  "type": "module",
  "scripts": {
...
// {{PROJECT_NAME}} — OriginDB WASM module (AssemblyScript).
//
// assembly/env.ts and assembly/index.ts are the OriginDB SDK (vendored from
// sdk/typescript in the OriginDB repository) — edit this file, not those.
//
// Build:   npm install && npm run asbuild   → build/module.wasm
// Publish: origindb publish
// =============================================================================
...
An OriginDB WASM module written in AssemblyScript (TypeScript-syntax
language compiled to WebAssembly). Reducers run inside the OriginDB server
with transactional table access (WASM ABI v1).

## Layout

- `assembly/module.ts` — **your module** (reducers, filters). Edit this.
- `assembly/index.ts`, `assembly/env.ts` — the OriginDB SDK, vendored from
  `sdk/typescript` in the OriginDB repository. Don't edit; re-copy to upgrade.
...
- A non-null reducer return value is serialized and returned via
  ...
  staged writes.
- See `sdk/typescript/README.md` in the OriginDB repository for the full
  API reference.
)TSREADME"},
...
public class GameManager : MonoBehaviour
{
    ...
    async void Start()
    {
        Debug.Log("Initializing OriginDB connection...");
    ...
    async Task ConnectToServer()
    {
        try
        {
            ...
            Debug.Log("Connected to OriginDB!");
        }
    ...
    {
        Debug.LogWarning($"Disconnected from OriginDB: {ex?.Message}");
    }
    ...
    void OnDestroy()
    {
        ...
        {"Assets/OriginDBConfig.asset.meta", R"(fileFormatVersion: 2
guid: YOUR_GUID_HERE
...
An OriginDB Unity multiplayer game project.

## Getting Started

1. Install the OriginDB Unity package:
   ```bash
   # In Unity Package Manager, add from git URL:
   https://github.com/origindb/origindb-unity.git
   ```
...
4. Configure the OriginDB settings in the Inspector

5. Play the scene!

## Project Structure

- `Assets/Scripts/GameManager.cs` - Main game manager with OriginDB integration
- `Assets/OriginDBConfig.asset` - Connection configuration (create via menu)
- `origindb.config.json` - Server-side configuration

## Creating the Config Asset

1. Right-click in Project window
2. Create → OriginDB → Configuration
3. Configure your server settings
4. Assign to GameManager

## Documentation

- [OriginDB Unity Guide](https://docs.origindb.com/unity)
- [API Reference](https://docs.origindb.com/api)
)"},
        {".gitignore", R"(# Unity
[Ll]ibrary/
[Tt]emp/
...
logs/
    ...
    {"nodejs", {
        {"index.js", R"DELIMITER(const { OriginDBClient } = require('@origindb/client');

async function main() {
  console.log('🚀 Connecting to OriginDB...');

  // Create client
  const client = new OriginDBClient({
    serverUrl: 'http://localhost:8080',
    moduleName: '{{PROJECT_NAME_LOWER}}'
  });
...
main().catch(console.error);
  ...
  "name": "{{PROJECT_NAME_LOWER}}",
  "version": "1.0.0",
  "description": "An OriginDB Node.js project",
  "main": "index.js",
  "scripts": {
    "start": "node index.js",
...
- [OriginDB Documentation](https://docs.origindb.com)
- [Node.js Client SDK](https://docs.origindb.com/nodejs)
)"},
...
void printUsage() {
    std::cout << BOLD << "Usage:" << RESET << " origindb init [PROJECT_NAME] [OPTIONS]\n\n";
    ...
    std::cout << "  origindb init myproject                       # C# WASM module\n";
    std::cout << "  origindb init mymodule --template typescript  # AssemblyScript WASM module\n";
    std::cout << "  origindb init mygame --template unity         # Unity project\n";
    std::cout << "  origindb init myapp --template nodejs         # Node.js project\n";
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << CYAN << BOLD << "\n🚀 Initializing OriginDB " << templateName << " project: "
              << projectName << RESET << "\n\n";
    ...
    if (templateName == "csharp") {
        std::cout << "  2. Copy the OriginDB C# SDK into the project (no NuGet package yet):\n";
        std::cout << "     " << BLUE << "cp /path/to/instant_db/sdk/csharp/OriginDB.cs ." << RESET << "\n";
        ...
        std::cout << "  5. Publish to a running OriginDB server:\n";
        std::cout << "     " << BLUE << "origindb publish" << RESET << "\n";
        ...
        std::cout << "  4. Publish to a running OriginDB server:\n";
        std::cout << "     " << BLUE << "origindb publish" << RESET << "\n";
    } else if (templateName == "unity") {
        std::cout << "  2. Open the project in Unity\n";
        std::cout << "  3. Install the OriginDB Unity package\n";
        std::cout << "  4. Start the OriginDB server:\n";
        std::cout << "     " << BLUE << "origindb server" << RESET << "\n";
        std::cout << "  5. Play the scene in Unity\n";
    } else if (templateName == "nodejs") {
        std::cout << "  2. Install dependencies:\n";
        std::cout << "     " << BLUE << "npm install" << RESET << "\n";
        std::cout << "  3. Start the OriginDB server:\n";
        std::cout << "     " << BLUE << "origindb server" << RESET << "\n";
